...
struct map_static *alloc_map_static_mod()
{
	struct map_static *m;
...
void change_tcp_state(uint8_t flags, struct cache_entry *c_ptr, uint8_t ip_version)
{
 
	tcp_state initial_state = c_ptr->state;         
	switch(c_ptr->state)
	{
		case CLOSED:			
			if((flags & SYN) && (ip_version == IP6))
			{
				c_ptr->state = V6_INIT;
			}
		...
		break;
		case V6_INIT:			
			if((flags & SYN) && (ip_version == IP4))
			{				
				c_ptr->state = ESTABLISHED;
			}
		...
		case ESTABLISHED:			
			...
			{
				c_ptr->state = TRANS;
				c_ptr->last_use = now - CACHE_MAX_AGE + TRANS_TO;
			}
			else if((flags & FIN) && (ip_version == IP4))
			{
				c_ptr->state = V4_FIN_RECV;
			}
			else if((flags & FIN) && (ip_version == IP6))
			{
				c_ptr->state = V6_FIN_RECV;
			}
		break;
		case V4_FIN_RECV:			
			if((flags & FIN) && (ip_version == IP6))
			{
				c_ptr->state = V4_FIN_V6_FIN_RECV;				
				c_ptr->last_use = now - CACHE_MAX_AGE + TRANS_TO;		
			}
		break;
		case V6_FIN_RECV:			
			if((flags & FIN) && (ip_version == IP4))
			{
				c_ptr->state = V4_FIN_V6_FIN_RECV;
				c_ptr->last_use = now - CACHE_MAX_AGE + TRANS_TO;	
			}
		break;
		case V4_FIN_V6_FIN_RECV:
		break;
		case TRANS:			
			c_ptr->last_use = now - CACHE_MAX_AGE + TRANS_TO;
		break;
	...
	char addrbuf4[INET6_ADDRSTRLEN];
	inet_ntop(AF_INET, &(c_ptr->addr4), addrbuf4, sizeof(addrbuf4));
	inet_ntop(AF_INET6, &(c_ptr->addr6), addrbuf6, sizeof(addrbuf6));

	slog(LOG_INFO, "\nIP6 %s %d\nIP4 %s %d\nInitial State %d\nFinal State %d\n\n\n", addrbuf6, c_ptr->port_addr6, addrbuf4, c_ptr->port_addr4, initial_state, c_ptr->state);	
}
...
static struct cache_entry *cache_insert(const struct in_addr *addr4,
		...
		uint32_t hash4, uint32_t hash6, uint16_t port_addr4, uint16_t port_addr6, tcp_state state)
{
	struct cache_entry *c;
	...
	c->addr4 = *addr4;
	c->addr6 = *addr6;
	c->port_addr4 = port_addr4;
	c->port_addr6 = port_addr6;
	c->last_use = now;
	c->flags = 0;
	c->ip4_ident = 1;
	c->state = state;
...
int map_ip4_to_ip6(struct in6_addr *addr6, const struct in_addr *addr4,
		struct cache_entry **c_ptr, struct transport_buf trans_buf)
{
	uint32_t hash;
	struct list_head *entry;
	struct cache_entry *c;
	...
	if (gcfg->cache_size) {
		...
		list_for_each(entry, &gcfg->hash_table4[hash]) {
			...
			if (addr4->s_addr == c->addr4.s_addr) {
				if(trans_buf.port_v4 != NULL && c->port_addr4 == *(trans_buf.port_v4))
				{					
					*(trans_buf.port_v6) = c->port_addr6;
					*addr6 = c->addr6;				
					c->last_use = now;
					if (c_ptr)
						*c_ptr = c;						
					if(trans_buf.proto == TCP)
						change_tcp_state(trans_buf.flags, c, IP4);
					return 0;
				}
				else if(trans_buf.port_v4 == NULL) {
					*addr6 = c->addr6;				
					if (c_ptr)
						*c_ptr = c;
					if(trans_buf.proto == TCP)
	...
	trans_buf.port_v4 = (uint16_t *)malloc(sizeof(uint16_t));
	*(trans_buf.port_v4) = 0;
	...
	if (gcfg->cache_size) {
		c = cache_insert(addr4, addr6, hash, hash_ip6(addr6), *(trans_buf.port_v4), *(trans_buf.port_v6), CLOSED);
...
int map_ip6_to_ip4(struct in_addr *addr4, const struct in6_addr *addr6,
		struct cache_entry **c_ptr, int dyn_alloc, struct transport_buf trans_buf)
{
	uint32_t hash;
	struct list_head *entry;
	struct cache_entry *c;
	...
	if (gcfg->cache_size) {
		...
		list_for_each(entry, &gcfg->hash_table6[hash]) {
			...
			{				
				if(trans_buf.port_v6 != NULL && c->port_addr6 == *(trans_buf.port_v6))          			
				{					
					*(trans_buf.port_v4) = c->port_addr4;
					*addr4 = c->addr4;
					c->last_use = now;
					if (c_ptr)
						*c_ptr = c;					
					if(trans_buf.proto == TCP)
						change_tcp_state(trans_buf.flags, c, IP6);
					return 0;
				}
				else if(trans_buf.port_v6 == NULL) {
					*addr4 = c->addr4;
					c->last_use = now;
					if (c_ptr)
						*c_ptr = c;
	...
	/* generate port number */
		...
		while(port_create) {
			port_create = 0;
			random = (uint16_t)(50000 +  rand() % 10000);
			list_for_each(port_list_head, &gcfg->port_list) {
				pl = list_entry(port_list_head, struct port_list, list);
				if(pl->port == random) {
					port_create = 1;
					break;
				}
		...
		port_list_head = &gcfg->port_list;
		pl = (struct port_list *)malloc(sizeof(struct port_list));
		INIT_LIST_HEAD(&pl->list);
		pl->port = random;
		list_add(&pl->list, port_list_head);	
		*(trans_buf.port_v4) = htons(random);
		if(dyn_alloc == 0)
			*(trans_buf.port_v4) = *(trans_buf.port_v6);
	}	
	...
	if (!map6) {		
		...
		{			
			temp_s = alloc_map_static_mod();
			temp_s->map4.addr = gcfg->default_addr4;
			map6 = &temp_s->map6;			
		}
		if (!map6)
			return -1;	
	...
	if (gcfg->cache_size) {
		c = cache_insert(addr4, addr6, hash_ip4(addr4), hash_ip6(addr6), *(trans_buf.port_v4), *(trans_buf.port_v6), CLOSED);
		if (c_ptr)
			*c_ptr = c;
		if (d) {
			d->cache_entry = c;
	...
	if(trans_buf.proto == 6)
		change_tcp_state(trans_buf.flags, c, 6);
	return 0;
}
...
void addrmap_maint(void)
{
	...
	list_for_each_safe(entry, next, &gcfg->cache_active) {
		...
		if (c->last_use + CACHE_MAX_AGE < now) {
			...
			char addrbuf6[INET6_ADDRSTRLEN];
			char addrbuf4[INET6_ADDRSTRLEN];
			inet_ntop(AF_INET, &(c->addr4), addrbuf4, sizeof(addrbuf4));
			inet_ntop(AF_INET6, &(c->addr6), addrbuf6, sizeof(addrbuf6));
			slog(LOG_INFO, "\nIP6 %s %d\nIP4 %s %d\nFinal State %d\n", addrbuf6, ntohs(c->port_addr6), addrbuf4, ntohs(c->port_addr4), c->state);			              
			slog(LOG_INFO,"Cache Entry Removed\n\n\n");
		}
	}
}
