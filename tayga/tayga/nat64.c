...
static uint16_t change_port(struct pkt *p, uint16_t * tck, uint16_t *port1, uint16_t offset)
{
	uint16_t *port2 = (uint16_t *)(p->data + offset);
	*tck = ~*tck;		
	*tck = ones_add(*tck, ~(*port2));
	*tck = ones_add(*tck, *port1);	
	*port2 = *port1;	
	return ~*tck;
}
...
static int xlate_payload_4to6(struct pkt *p, struct ip6 *ip6, uint16_t * dest_port_addr6)
{
	uint16_t *tck;
	uint16_t cksum;
	...
	case 17:
		...
		*tck = change_port(p, tck, dest_port_addr6, 2);
		break;
	case 6:
		if (p->data_len < 20)
			return -1;
...
static void xlate_4to6_data(struct pkt *p)
{
	...
	int frag_size;
	struct transport_buf trans_buf;
	...
	trans_buf.port_v4 = (uint16_t *)(p->data + 2);
	trans_buf.port_v6 = (uint16_t *)malloc(sizeof(uint16_t));
	trans_buf.proto = p->data_proto;	
	(p->data_proto == 6)? trans_buf.flags = *(p->data + 13) : 0;	 	
	
	if (p->data_proto == 6 || p->data_proto == 17) 
	{		
		if (map_ip4_to_ip6(&header.ip6.dest, &p->ip4->dest, &dest, trans_buf)) 
		{
			host_send_icmp4_error(3, 1, 0, p);
			return;
		}		
		trans_buf.port_v4 = 0;
		if (map_ip4_to_ip6(&header.ip6.src, &p->ip4->src, &src, trans_buf)) 
	{
	...
	if (xlate_payload_4to6(p, &header.ip6, trans_buf.port_v6) < 0)
		return;
...
static void xlate_4to6_icmp_error(struct pkt *p)
{
	...
	if (map_ip4_to_ip6(&header.ip6_em.src, &p_em.ip4->src, NULL, trans_buf) ||
			map_ip4_to_ip6(&header.ip6_em.dest,
					&p_em.ip4->dest, &orig_dest, trans_buf))
		return;
	...
	if (xlate_payload_4to6(&p_em, &header.ip6_em, NULL) < 0)
		return;

	if (map_ip4_to_ip6(&header.ip6.src, &p->ip4->src, NULL, trans_buf)) {
	...
	if (map_ip4_to_ip6(&header.ip6.dest, &p->ip4->dest, NULL, trans_buf))
		return;
...
static int xlate_payload_6to4(struct pkt *p, struct ip4 *ip4, uint16_t * src_port_addr4)
{
	uint16_t *tck;
	uint16_t cksum;
	...
	case 17:
		...
		if (!*tck)
			return -1; /* drop UDP packets with no checksum */
		*tck = change_port(p, tck, src_port_addr4, 0);
		break;
	case 6:
		if (p->data_len < 20)
			return -1;
...
static void xlate_6to4_data(struct pkt *p)
{
	...
	trans_buf.port_v6 = (uint16_t *)(p->data);
	...
	*(trans_buf.port_v4) = 0;
	...
	trans_buf.port_v6 = (uint16_t *)(p->data + 2);
	if (map_ip6_to_ip4(&header.ip4.dest, &p->ip6->dest, &dest, 0, trans_buf)) {
		host_send_icmp6_error(1, 0, 0, p);
		return;
	}	

	trans_buf.port_v6 = (uint16_t *)(p->data);
	if (map_ip6_to_ip4(&header.ip4.src, &p->ip6->src, &src, 2, trans_buf)) {
	...
	if (xlate_payload_6to4(p, &header.ip4, trans_buf.port_v4) < 0)
		return;
...
static void xlate_6to4_icmp_error(struct pkt *p)
{
	...
	if (map_ip6_to_ip4(&header.ip4_em.src, &p_em.ip6->src, NULL, 0, trans_buf) ||
			map_ip6_to_ip4(&header.ip4_em.dest,
						&p_em.ip6->dest, NULL, 0, trans_buf) ||
			xlate_payload_6to4(&p_em, &header.ip4_em, NULL) < 0)
		return;
	...
	if (map_ip6_to_ip4(&header.ip4.src, &p->ip6->src, NULL, 0, trans_buf)) {
		if (allow_fake_source)
	...
	if (map_ip6_to_ip4(&header.ip4.dest, &p->ip6->dest, NULL, 0, trans_buf))
		return;
