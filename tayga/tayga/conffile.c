...
static void config_default_addr4(int ln, int arg_count, char **args)
{
	if(inet_pton(AF_INET, args[0], &(gcfg->default_addr4)) < 0)
	{
		slog(LOG_CRIT, "Error: invalid default IPv4 address\n");
		exit(1);
	}	
}
...
struct {
	...
	{ "default-addr4", config_default_addr4, 1},
	{ NULL, NULL, 0 }
};
